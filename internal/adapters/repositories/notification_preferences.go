...
func (r *NotificationPreferencesRepository) GetByUserID(ctx context.Context, userID uuid.UUID) (*domain.NotificationPreferences, error) {
	query := `
		SELECT user_id, like_review, like_comment, new_comment, new_follow, system, achievement_unlocked, updated_at
		FROM notification_preferences WHERE user_id = $1
	`
	prefs := &domain.NotificationPreferences{}
	err := r.db.Pool.QueryRow(ctx, query, userID).Scan(
		&prefs.UserID, &prefs.LikeReview, &prefs.LikeComment,
		&prefs.NewComment, &prefs.NewFollow, &prefs.System,
		&prefs.AchievementUnlocked, &prefs.UpdatedAt,
	)
...
func (r *NotificationPreferencesRepository) Upsert(ctx context.Context, prefs *domain.NotificationPreferences) error {
	query := `
		INSERT INTO notification_preferences (user_id, like_review, like_comment, new_comment, new_follow, system, achievement_unlocked, updated_at)
		...
		ON CONFLICT (user_id) DO UPDATE SET
			...
			achievement_unlocked = EXCLUDED.achievement_unlocked,
